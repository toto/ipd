...
#endif
    pd_init();                                  /* start the message system */			//start here!@!!!!!
    sys_findprogdir(argv[0]);                   /* set sys_progname, guipath */			//searches for first cmd argument! need to replace this with resources sandbox!
...
#endif
    if (sys_argparse(argc-1, argv+1))           /* parse cmd line */					//assuming everything
        return (1);																		//after is optional
...
void sys_findprogdir(char *progname)
{
    ...
    char *lastslash; 
#ifdef UNISTD							//iPhone is darwin based! :D
    struct stat statbuf;
#endif
